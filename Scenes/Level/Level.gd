...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("explode"):
		SignalHub.emit_on_create_powerup(Vector2(300, 300), PowerUp.PowerUpType.Shield)
		#SignalHub.emit_on_create_bullet(Vector2(300, 300), Vector2.DOWN, Bullet.BulletType.EnemyBullet, 100)
